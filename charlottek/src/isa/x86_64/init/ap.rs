...
static AP_INTERRUPT_STACKS: Lazy<Vec<[u8; INTERRUPT_STACK_SIZE]>> = Lazy::new(|| {
    ...
    logln!("LP{}: Allocating {} AP interrupt stacks.", (LpControl::get_lp_id()), num_aps);
    let mut ret = Vec::<[u8; INTERRUPT_STACK_SIZE]>::with_capacity(num_aps as usize);
    for _ in 0..num_aps {
        ret.push(*(Box::new([0u8; INTERRUPT_STACK_SIZE])));
    }
    logln!("LP{}: AP interrupt stacks allocated.", (LpControl::get_lp_id()));
    ret
});
...
static AP_DF_STACKS: Lazy<Vec<[u8; INTERRUPT_STACK_SIZE]>> = Lazy::new(|| {
    ...
    let num_aps = get_lp_count() - 1; // Exclude BSP
    logln!("LP{}: Allocating {} AP df stacks.", (LpControl::get_lp_id()), num_aps);
    let mut ret = Vec::<[u8; INTERRUPT_STACK_SIZE]>::with_capacity(num_aps as usize);
    for _ in 0..num_aps {
        ret.push(*(Box::new([0u8; INTERRUPT_STACK_SIZE])));
    }
    logln!("LP{}: AP df stacks allocated.", (LpControl::get_lp_id()));
    ret
});

static AP_TSS: Lazy<Vec<super::gdt::Tss>> = Lazy::new(|| {
    logln!("LP{}: Creating the TSS vector.", (LpControl::get_lp_id()));
    let mut tsses = Vec::new();
    logln!("LP{}: Allocating {} TSS entries.", (LpControl::get_lp_id()), (get_lp_count() - 1));
    ...
    logln!("LP{}: TSS vector initialized.", (LpControl::get_lp_id()));
    tsses
});

static AP_GDTS: Lazy<Vec<super::gdt::Gdt>> = Lazy::new(|| {
    logln!("LP{}: Creating the GDT vector.", (LpControl::get_lp_id()));
    let mut gdts = Vec::new();
    logln!("LP{}: Allocating {} GDT entries.", (LpControl::get_lp_id()), (get_lp_count() - 1));
    for tss in AP_TSS.iter() {
        logln!("LP{}: Constructing and allocating a GDT", (LpControl::get_lp_id()));
        gdts.push(super::gdt::Gdt::new(tss));
    }
    logln!("LP{}: GDT vector initialized.", (LpControl::get_lp_id()));
    gdts
});

static AP_IDTS: Lazy<Vec<crate::isa::interrupts::idt::Idt>> = Lazy::new(|| {
    logln!("LP{}: Creating the IDT vector.", (LpControl::get_lp_id()));
    let mut idts = Vec::new();
    logln!("LP{}: Allocating {} IDT entries.", (LpControl::get_lp_id()), (get_lp_count() - 1));
    for _ in 0..(get_lp_count() - 1) {
        logln!("LP{}: Constructing and allocating an IDT", (LpControl::get_lp_id()));
        let mut idt = crate::isa::interrupts::idt::Idt::new();
        logln!("LP{}: Registering fixed interrupt gates.", (LpControl::get_lp_id()));
        crate::isa::interrupts::register_fixed_isr_gates(&mut idt);
        logln!("LP{}: Pushing the initialized IDT to the vector.", (LpControl::get_lp_id()));
        idts.push(idt);
    }
    logln!("LP{}: IDT vector initialized.", (LpControl::get_lp_id()));
    idts
});

pub fn init_ap() {
    let lp_id = crate::isa::lp::LpControl::get_lp_id();
