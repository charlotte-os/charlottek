...
fn tlb_shootdown(asid: AddressSpaceId, vaddr: VAddr, page_count: u16) {
    let page_base = vaddr.page_offset();

    if asid == 0 && CpuInfo::is_extension_supported(IsaExtension::Invlpgb) {
        ...
        todo!("main TLB shootdown sequence")
    }
}
